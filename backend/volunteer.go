...
func (s *Server) verifyVolunteerSign(c *gin.Context) {
	id := c.Param("id")
	var req struct {
		Effective      bool   `json:"effective" binding:"required"`
	...
	defer tx.Rollback()
	var orderID, elderID int
	var orderNo, eff string
	err = tx.QueryRow(`SELECT d.order_id, d.effectiveness, o.elder_id, o.order_no
		FROM deliveries d JOIN orders o ON o.id=d.order_id WHERE d.id=$1 FOR UPDATE OF d`, id).
		Scan(&orderID, &eff, &elderID, &orderNo)
	if err != nil {
		fail(c, http.StatusNotFound, "配送记录不存在")
		return
	}
	// 记录老人本人/同住人回访
	tx.Exec(`INSERT INTO elder_confirmations(order_id, elder_id, confirmer_role, confirmer_name, method,
		...
		req.Effective && !req.ReceiptDispute, req.TasteFeedback, req.BodyDiscomfort, req.ReceiptDispute, req.Note, uid)
	if req.Effective && !req.ReceiptDispute {
		...
		tx.Exec(`UPDATE orders SET status='signed', sign_effectiveness='valid', sign_basis='community', updated_at=now() WHERE id=$1`, orderID)
		tx.Exec(`UPDATE anomalies SET status='resolved', responsible_party='none', investigation=$2, outcome='none',
		...
		addOrderEvent(tx, orderID, uid, c.GetString("name"), "社区核实签收有效", "回访"+map[string]string{"elder": "老人本人", "cohabitant": "同住人"}[req.ConfirmerRole]+"确认收到用餐")
	} else {
		tx.Exec(`UPDATE deliveries SET effectiveness='invalid', effectiveness_reason='社区回访否认收到或签收异常',
			community_verified_by=$2, community_verified_at=now() WHERE id=$1`, id, uid)
		tx.Exec(`UPDATE orders SET status='exception', sign_effectiveness='invalid', updated_at=now() WHERE id=$1`, orderID)
		// 否认收到 => 志愿者帮送异常，待进一步划分责任
		anID, _ := createAnomaly(tx, orderID, elderID, "volunteer_delivery",
			"社区回访中老人/同住人否认收到或签收异常："+req.Note, uid)
		tx.Exec(`UPDATE anomalies SET elder_unwell=$2 WHERE id=$1`, anID, req.BodyDiscomfort)
		addOrderEvent(tx, orderID, uid, c.GetString("name"), "社区核实签收无效", "老人否认收到/签收异常，转异常处理")
	}
	...
	ok(c, gin.H{"verified": true, "effective": req.Effective && !req.ReceiptDispute})
}
...
func (s *Server) elderConfirmation(c *gin.Context) {
	...
	if _, err := tx.Exec(`INSERT INTO elder_confirmations(order_id, elder_id, confirmer_role, confirmer_name, method,
		...
		orderID, elderID, req.ConfirmerRole, req.ConfirmerName, req.Method,
		req.ConfirmsReceived && !req.ReceiptDispute, req.TasteFeedback, req.BodyDiscomfort, req.ReceiptDispute, req.Note, uid); err != nil {
		fail(c, http.StatusInternalServerError, "记录回访失败")
		return
	}
	addOrderEvent(tx, orderID, uid, c.GetString("name"), "回访老人本人/同住人",
		"确认收到："+map[bool]string{true: "是", false: "否"}[req.ConfirmsReceived && !req.ReceiptDispute]+
	...
	if req.ConfirmsReceived && !req.ReceiptDispute && status == "verify_pending" {
		tx.Exec(`UPDATE orders SET status='signed', sign_effectiveness='valid', sign_basis='community', updated_at=now() WHERE id=$1`, orderID)
	...
	if req.ReceiptDispute {
		anID, _ := createAnomaly(tx, orderID, elderID, "volunteer_delivery",
			"家属代订单 "+orderNo+" 回访老人本人/同住人时否认收到或签收异常："+req.Note, uid)
		_ = anID
	}
	if req.BodyDiscomfort {
